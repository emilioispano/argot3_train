...
def get_args():
    ...
    parser.add_argument('-f', '--kfold', required=False, default=4, type=int)
    ...
    parser.add_argument('-1', '--fold', required=False, default=0, type=int)
    parser.add_argument('-2', '--epoch', required=False, default=0, type=int)
...
def write_folds(folds):
    for i, fold in enumerate(folds):
        with open(f'intermediate_files/amr/folds/fold_{i}.txt', 'w') as fp:
            for prot in fold:
                fp.write(f'{prot}\n')
...
def model_checkpoint(model_summary, TP, FP, FN, fold, epoch):
    ...
    with open(model_summary, 'a') as out:
        ...
        max_pr = pr[max_th]
        max_rc = rc[max_th]
        out.write(f'Fold: {fold + 1}, epoch: {epoch + 1}. F_max: {max_f:.4f}, precision: {max_pr:.4f}, recall: {max_rc:.4f} (th: {max_th})\n')
...
if __name__ == '__main__':
    ...
    epoch_resume = args['epoch']
    fold_resume = args['fold']

    print('Loading labels...')
    with open(labels_file, 'r') as fp:
        labels = {prot: int(label) for prot, label in (line.strip().split('\t') for line in tqdm(fp, total=392000))}
    prots = set(labels.keys())  # Compute this only once after reading the file

    print('Managing folds...')
    resume = True if epoch_resume else False
    if resume:
        print('Resume mode')
        folds = load_folds(k_folds)
    else:
        folds = get_folds(prots, 1 / (k_folds + 1))
        write_folds(folds)
    ...
    for fold in range(len(folds)):
        if resume and fold < fold_resume:
            print(f'Fold {fold + 1}/{len(folds)} already trained, skipping...')
            continue

        train_prots = list(chain.from_iterable(folds[:fold] + folds[fold+1:]))
        val_prots = folds[fold]

        num_train_batches = int(len(train_prots) / batch_size)
        num_val_batches = int(len(val_prots) / batch_size)
        ...
        for epoch in range(num_epochs):
            ...
            if resume:
                model.load_weights(model_resume)
                resume = False
            else:
                if epoch > 0:
                    model.load_weights(os.path.join(save_path, f'amr_{fold}_{epoch - 1}.h5'))

            print(f'Fold: {fold + 1} / {len(folds)}. Epoch {epoch + 1} / {num_epochs}, training.')
            ...
            model.fit(train_generator, epochs=1, steps_per_epoch=num_train_batches)

            model.save_weights(os.path.join(save_path, f'amr_{fold}_{epoch}.h5'))

            val_generator = batches_generator_buffer(val_prots, labels, embed_path, batch_size, buffer_size)
            Y_true, Y_pred = [], []

            with tqdm(range(num_val_batches), total=num_val_batches, desc='Validating...') as pbar:
                for X_val, Y_val in val_generator:
                    y_pred = model.predict_on_batch(X_val)

                    y_pred = y_pred.flatten().tolist()
                    Y_val = Y_val.numpy().flatten().tolist()
            ...
            model_checkpoint(os.path.join(save_path, f'amr_recap.txt'), TP, FP, FN, fold, epoch)
