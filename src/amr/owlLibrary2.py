...
class GoOwl:
    ...
    def __init__(self, owl, namespace='', goa_file='', by_ontology=False, use_all_evidence=True,
                 valid_evidence=('EXP', 'IDA', 'IPI', 'IMP', 'IGI', 'IEP', 'TAS', 'IC'), edges=('is a', 'part of')):
        self.__owl = owl
        self.__ns = namespace
        self.__global = {}
        self.__global_total = {}
    ...
    def obsolete_deprecated_new(self):
        self.__obsolete_new, self.__deprecated_new = set(), set()
        if self.__file_extension == 'owl':
            for go, go_name_son in self.__global_total.items():
                if go_name_son.name.startswith('GO_'):
                    #  deprecated node
                    if not go_name_son.label.first():
                        self.__deprecated_new.add(go_name_son.name)

                    #  obsolete node
                    else:
                        if go_name_son.label.first().startswith("obsolete"):
                            self.__obsolete_new.add(go_name_son.name)
    ...
    def get_obsolete_deprecated_set(self):
        return self.__obsolete_new, self.__deprecated_new
    #  END DEF
